...
- **TPTP → Isabelle.** `Isabelle.query_tptp/2` / `prove_tptp/3` route TPTP/THF problems through `IsabelleClient.TPTP.isabellize_theory/1` and prepend `unbundle from_TPTP`. The bundled `TPTP.thy` is copied into `:local_dir` on first use. Lemma `:line` numbers in the response refer to the **generated** theory, not the input TPTP — UIs should attribute by `:name` (which is propagated from the TPTP formula name). `Isabelle.query/2` (the `Backend` entry point) wraps `query_tptp/2` and collapses the per-lemma list weakest-link: the unified result is `{:ok, :thm}` only if every isabellized lemma was discharged. The three-arity `query/3` for hand-written theories still exists, but its `opts` argument is mandatory (pass `[]` if none) so the arities don't collide.
